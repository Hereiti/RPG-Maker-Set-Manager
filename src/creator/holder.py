...
from PySide6.QtWidgets import (
    ...
    QToolBar, QDialog, QCheckBox, QGridLayout, QPushButton, QComboBox, QLabel)
    ...
    for index, filename in enumerate(images):
        ...
        for index, item in enumerate(index_list):
            ...
            copied_image = image.copy(
                *maths.cell_origin(item), cell_width, cell_height)
            painter.drawPixmap(
...
def add_weapon_layer(app, file_path, layer):
    ...
    if app.layer_1:
        for item in app.layer_1:
            app.main_view.scene.removeItem(
                grid_manager.get_layer_1(app, item)[0])
...
def pixel_comparaison(image):
    ...
    characters = Image.open("./res/characters.png")
...
def tree_view(app):
    response = utils.show_popup(textwrap.dedent("""
        ...
        <br>folder. If you want to make the process quicker, it's recommended to create
    ...
    if app.toolbar:
        app.removeToolBar(app.toolbar)
        app.toolbar.deleteLater()
        app.tree_view.deleteLater()

    else:
        app.setFixedWidth(app.width() + 300)
    ...
    app.tree_view.setFixedSize(800, 600)
    ...
    app.toolbar = QToolBar()
    app.toolbar.setMovable(False)
    app.toolbar.setFixedSize(300, app.height())
    app.toolbar.addWidget(label)
    app.toolbar.addWidget(app.tree_view)
    app.toolbar.addWidget(button_save)
