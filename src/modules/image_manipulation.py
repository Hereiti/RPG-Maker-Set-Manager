...
def zoom(app, item, layer=0, reset=True):
    ...
    if item is not None:
        ...
        if config.config_get("TYPE") == "Weapons":
            image = image.resize((300, 220), resample=Image.NEAREST)
        else:
...
def play_animation(app):
    ...
    if (not app.main_view.highlight_selected
            and not app.main_view.mass_highlight):
        return

    if not app.images:
        return

    if len(app.main_view.mass_highlight) > 1:
        indexes = []
        for highlight, index in app.main_view.mass_highlight:
            indexes.append(index)

        app.animation[1] = indexes
    ...
    def animation_timeout():
        ...
        grid_manager.highlight_index(
            app, app.animation[1][image_index])
...
def change_hue(app):
    ...
    if len(app.main_view.mass_highlight) > 0:
        ...
        for index in indexes:
            ...
            # Wait for the slider to reach the end position
            while not hue_slider.slider_end:
                loop.exec_()

            hue_slider.slider_end = False
    else:
...
def change_saturation(app):
    ...
    Args:
        - app: The application main window
    """
    value = app.values["Saturation"].text()
    if not value.isdigit():
        return
    value = int(value)
    ...
    if len(app.main_view.mass_highlight) > 0:
        ...
        for index in indexes:
            ...
            # Wait for the slider to reach the end position
            while not saturation_slider.slider_end:
                loop.exec_()

            saturation_slider.slider_end = False
    else:
        image = grid_manager.get_image_at(
            app, app.main_view.highlight_selected[1])
        if image is None:
...
def change_value(app):
    ...
    if len(app.main_view.mass_highlight) > 0:
        ...
        for index in indexes:
            ...
            # Wait for the slider to reach the end position
            while not value_slider.slider_end:
                loop.exec_()

            value_slider.slider_end = False
    else:
        image = grid_manager.get_image_at(
            app, app.main_view.highlight_selected[1])
        if image is None:
...
def flip_image(app, orientation):
    ...
    if app.main_view.highlight_selected[0]:
        images_index = [app.main_view.highlight_selected[1]]
    else:
        for highlight, index in app.main_view.mass_highlight:
...
def change_color(app, color=None):
    ...
    if len(app.main_view.mass_highlight) > 0:
        ...
        for index in indexes:
            ...
            slider = app.sliders[color]
            slider.setValue(value)
            slider.change_rgb(app, value, color.lower())
            slider.on_slider_end(app)

            loop = QEventLoop()

            # Wait for the slider to reach the end position
            while not slider.slider_end:
                loop.exec_()

            slider.slider_end = False
    else:
        image = grid_manager.get_image_at(
            app, app.main_view.highlight_selected[1])
        if image is None:
